...
while [[ -z "${EZA:-}" || "${EZA:-}" == "null" ]]; do
    EZA="$(curl -Ls https://api.github.com/repos/eza-community/eza/releases/latest | jq -r '.assets[] | select(.name | test(".*x86_64-unknown-linux-gnu.*gz")).browser_download_url')" || (true && sleep 30)
done
curl --retry 3 -L# "$EZA" | tar -xz -C /usr/local/bin/

# cosign
while [[ -z "${COSIGN:-}" || "${COSIGN}" == "null" ]]; do
    COSIGN="$(curl -Ls https://api.github.com/repos/sigstore/cosign/releases/latest | jq -r '.assets[] | select(.name | test(".*-linux-amd64$")).browser_download_url')" || (true && sleep 30)
done
...
while [[ -z "${DOCKER_FMT:-}" || "${DOCKER_FMT:-}" == "null" ]]; do
    DOCKER_FMT="$(curl -Ls https://api.github.com/repos/reteps/dockerfmt/releases/latest | jq -r '.assets[] | select(.name| test(".*linux-amd64.*gz$")).browser_download_url')" || (true && sleep 30)
done
...
while [[ -z "${EMACS_LSP_BOOSTER:-}" || "${EMACS_LSP_BOOSTER:-}" == "null" ]]; do
    EMACS_LSP_BOOSTER="$(curl -Ls https://api.github.com/repos/blahgeek/emacs-lsp-booster/releases/latest | jq -r '.assets[] | select(.name| test(".*musl.zip$")).browser_download_url')" || (true && sleep 30)
done
...
while [[ -z "${SYFT:-}" || "${SYFT:-}" == "null" ]]; do
    SYFT="$(curl -Ls https://api.github.com/repos/anchore/syft/releases/latest | jq -r '.assets[] | select(.name | test(".*_linux_amd64.*gz$")).browser_download_url')" || (true && sleep 30)
done
curl --retry 3 -L# "$SYFT" | tar -xz -C /usr/local/bin/

# yamlfmt
while [[ -z "${YAMLFMT:-}" || "${YAMLFMT:-}" == "null" ]]; do
    YAMLFMT="$(curl -Ls https://api.github.com/repos/google/yamlfmt/releases/latest | jq -r '.assets[] | select(.name | test(".*_Linux_x86_64.*gz$")).browser_download_url')" || (true && sleep 30)
done
